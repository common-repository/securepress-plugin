<?php
	...
	****************************************************************************************************/
    ...
    if(!class_exists('SecureLive_Scanner') && !isset($_ENV['SECURELIVE'])){
    	include('sl8.php');
	}
	...
	class SecureLive_Admin{
		...
		private $timeout = 3;
		private $url_cache_expires = 30;
		private $url_cache_mode = 3;
		private $url_cache_loaded = false;
		...
		public function SecureLive_Admin(){
			...
			//Cache Account
			$this->CacheAccountSettings();
		}
		public function CacheAccountSettings(){
			if($this->account->valid){
				$this->load_db_settings();
				$this->load_db();
				if($this->db_enabled()){
					$this->SetDBSetting("email_select", $this->account->email_select);
					$this->SetDBSetting("email_thresh", $this->account->email_thresh);
				}
				else if($this->is_joomla() || $this->is_WordPress()){
					$this->QueryDB_Init();
					//Create table if it doesn't exist
					$this->QueryDB("CREATE TABLE IF NOT EXISTS `sl_settings` (
					...
					#Save email_select
					{
						$key   = $this->QueryDB_Clean('email_select');
						$value = $this->QueryDB_Clean($this->account->email_select);
						
						//Does key exist?
						$q = "SELECT * FROM `sl_settings` WHERE `key` = '$key'";
						$rs = $this->QueryDB($q, true);
						...
						$this->QueryDB($q, false);
					}
					
					#Save email_thresh
					{
						$key   = $this->QueryDB_Clean('email_thresh');
						$value = $this->QueryDB_Clean($this->account->email_thresh);
		...
		public function config_exists(){
			...
			if(!@file_exists($this->config_fp)){
				$this->config_fp = str_replace("\\",'/',$_SERVER['DOCUMENT_ROOT']).'/../../securelive_max/sl_config.php';
				if(!@file_exists($this->config_fp)){//securelive_max config not found, use local
		...
		public function getVersionMessage(){
			$ver_msg = '';
	        $ver_html = $this->sl_post_request('current_version.php', 'file');
	    ...
	    public function is_WordPress(){
			...
			if(!defined('WP_PATH_BASE')){
				...
				if($pos === false)
					...
					define('WP_PATH_BASE', $addon = substr(__file__, 0, $pos));
			}

            $wpFile1 = WP_PATH_BASE.'wp-includes'.DS.'wp-db.php';
			$wpFile2 = WP_PATH_BASE.'wp-includes'.DS.'wp-diff.php';
            ...
            $is_wp_dir = (bool)substr_count($_SERVER['REQUEST_URI'],'/wp-content') || (bool)substr_count($_SERVER['REQUEST_URI'],'/wp-admin');
	    ...
	    public function ajax_response($w=false){
		    ...
		    elseif($what == 'url_cache_save'){
		    	$file = file_get_contents($this->config_fp);
		    	if($file === false){
		    		header("HTTP/1.0 404 Not Found");
		    		echo "Cannot locate config file.";
		    		return;
				}
				...
				$expires_saved   = false;
				$expires_string  = "//start_url_cache\n";
				$expires_string .= '$expires = '.intval(@$_POST['expires']).";\n";
				$expires_string .= "//end_url_cache\n";
				
				$timeout_saved   = false;
				$timeout_string  = "//start_timeout\n";
				$timeout_string .= '$timeout = '.intval(@$_POST['timeout']).";\n";
				$timeout_string .= "//end_timeout\n";
				
				$mode_saved   = false;
				$mode_string  = "//start_url_cache_mode\n";
				$mode_string .= '$cache_mode = '.intval(@$_POST['when']).";\n";
				$mode_string .= "//end_url_cache_mode\n";
				
				$db_lines  = "//start_db\n";
				$db_lines .= "//".$this->EncodeWhiteSpace(@$_POST['host'])."\n";
				$db_lines .= "//".$this->EncodeWhiteSpace(@$_POST['database'])."\n";
				$db_lines .= "//".$this->EncodeWhiteSpace(@$_POST['username'])."\n";
				$db_lines .= "//".$this->EncodeWhiteSpace(@$_POST['password'])."\n";
				$db_lines .= "//end_db\n";

				//Split config file before and after '//(start|end)_db'
				$conf_before_db = '';
				$conf_inside_db = '';
				$conf_after_db  = '';
				$pos = 'before';
				for($i = 0; $i < count($lines); $i++){
					...
					switch($pos){
						case 'before':
							if($line == '//start_url_cache'){
								if(isset($_POST['expires'])){
									$expires_saved = true;
									$conf_before_db .= $expires_string;
									$i+=2;//skip next 2 lines
								}
							}
							else if($line == '//start_timeout'){
								if(isset($_POST['timeout'])){
									$timeout_saved = true;
									$conf_before_db .= $timeout_string;
									$i+=2;//skip next 2 lines
								}
							}
							else if($line == '//start_url_cache_mode'){
								if(isset($_POST['when'])){
									$mode_saved = true;
									$conf_before_db .= $mode_string;
									$i+=2;//skip next 2 lines
								}
							}
							else if($line == '//start_db'){
								$pos = 'inside';
							}
							else{
								$conf_before_db .= $line . "\n";
				...
				#Insert DB settings
				if($pos == 'before'){//This settings section does not exist
					//insert at top
					$index = strpos($conf_before_db, '//start_bypass_arr');
					if($index !== false){
						$conf_before_db = substr($conf_before_db, 0, $index) . $db_lines . substr($conf_before_db, $index);
					}
				}
				else{//Settings where found
					$conf_inside_db .= $db_lines;
				}
				#Insert EXP if not updated
				if(!$expires_saved){
					$index = strpos($conf_before_db, "<?php");
					if($index !== false){
						$index += 5;
						$conf_before_db = substr($conf_before_db, 0, $index) . "\n" . $expires_string . substr($conf_before_db, $index);
					}
				}
				#Insert Timeout if not updated
				if(!$timeout_saved){
					$index = strpos($conf_before_db, "<?php");
					if($index !== false){
						$index += 5;
						$conf_before_db = substr($conf_before_db, 0, $index) . "\n" . $timeout_string . substr($conf_before_db, $index);
					}
				}
				#Insert Mode if not updated
				if(!$mode_saved){
					$index = strpos($conf_before_db, "<?php");
					if($index !== false){
						$index += 5;
						$conf_before_db = substr($conf_before_db, 0, $index) . "\n" . $mode_string . substr($conf_before_db, $index);
					}
				}
				file_put_contents($this->config_fp, trim($conf_before_db.$conf_inside_db.$conf_after_db, "\n"));
				echo "Saved URL Cache Settings.";
			}
			elseif($what == 'url_cache_clear_exp'){
				#works for any addon domains
				$this->load_db();
				$now = time();
				$q  = "DELETE FROM `sl_url_cache` WHERE `exp` < $now";
				$q2 = "OPTIMIZE TABLE `sl_url_cache`";
				...
				if($this->db_enabled()){
					...
					$this->QueryDB($q, false);
					$this->QueryDB($q2, false);
				}
				echo "EXP Cache Cleared";
			}
			elseif($what == 'url_cache_clear_all'){
				#does not work for addon domains, must perform this on that domain (link to same DB if not sharing config file)
				$this->load_db();
				$now = time();
				
				
				if($this->db_enabled()){
					$host = $this->db2->clean($this->account->domain);
					$q  = "DELETE FROM `sl_url_cache` WHERE `host` LIKE '%$host'";
					$q2 = "OPTIMIZE TABLE `sl_url_cache`";
					$this->db2->query($q);
					$this->db2->query($q2);
					$this->db2->stop();
					...
					$this->QueryDB_Init();
					$host = $this->QueryDB_Clean($this->account->domain);
					$q  = "DELETE FROM `sl_url_cache` WHERE `host` LIKE '%$host'";
					$q2 = "OPTIMIZE TABLE `sl_url_cache`";
					$this->QueryDB($q, false);
					$this->QueryDB($q2, false);
					echo "Cleared CMS cache";
				}
				else{
					echo "NO DB";	
				}
				
				echo htmlentities($host)." - Cache Cleared";
			}
			elseif($what == 'url_cache_records'){
				#Shows addon domain cache
				$this->load_db();
				
				$limit = isset($_COOKIE['limit']) ? intval($_COOKIE['limit']) : 15;
				if($limit == 0) $limit = 15;
				$limit_start = isset($_POST['Page']) ? intval($_POST['Page']) : 0;
				
				$q = "SELECT `id`,  IF(safe = 1, 'Yes', '<span style=\"color: red;\">No</span>') as safe_str,  CONCAT(`host`,`uri`), `threat_type`, `threat_level`, `ip`, DATE(FROM_UNIXTIME(`time`)), DATE(FROM_UNIXTIME(`exp`)), `blocks`  
						FROM `sl_url_cache`
						ORDER BY `time` DESC
				...
				if($this->db_enabled()){
					...
					echo "<tr><td colspan=\"100\">No database connection could be made.</td></tr>";
					return;	
				}
				
				$q = "SELECT COUNT(*) FROM `sl_url_cache`";
				if($this->db_enabled()){
					$rs_full_count = $this->db2->query($q);
				}
				else if($this->is_joomla() || $this->is_WordPress()){
					$rs_full_count = $this->QueryDB($q, true);
				}
				$rs_full_count = intval(@$rs_full_count[0][0]);
				
				$rs_count = count($rs);
			    $output = '<tr> <th style="width:  20px;"></th>
								...
								<th style="width:  70px;"></th></tr>';//For sizing

				#Loop though records
				for($r = 0; $r < $rs_count; $r++){
					$bg_color = $r%2==1 ? ' style="background-color:#eee;"' : '';
					$output .= "\n<tr class=\"cache_record_".intval($r & 1)."\"$bg_color>";
					$f_count = count($rs[$r]);
					$blocks = explode('|',$rs[$r][8]);
					$blocks = intval(@$blocks[0]);
					
					$output .= "<td><input type=\"checkbox\" value=\"".intval($rs[$r][0])."\" name=\"cache_ids\" /></td>";
					$output .= "<td><div class=\"div_cache_field_0\">".htmlentities($rs[$r][0])."</div></td>";
					$output .= "<td><div class=\"div_cache_field_1\">".$rs[$r][1]."</div></td>";//SKIP htmlentities. Check query statement.
					$output .= "<td><div class=\"div_cache_field_8\">".htmlentities($blocks)."</div></td>";
					$output .= "<td><div class=\"div_cache_field_2\">".htmlentities($rs[$r][2])."</div></td>";
					$output .= "<td><div class=\"div_cache_field_3\">".htmlentities($rs[$r][3])."</div></td>";
					$output .= "<td><div class=\"div_cache_field_4\">".htmlentities($rs[$r][4])."</div></td>";
					$output .= "<td><div class=\"div_cache_field_5\">".htmlentities($rs[$r][5])."</div></td>";
					$output .= "<td><div class=\"div_cache_field_6\">".htmlentities($rs[$r][6])."</div></td>";
					$output .= "<td><div class=\"div_cache_field_7\">".htmlentities($rs[$r][7])."</div></td>";
					$output .= "</tr>\n";
				}
				
				#Create Footer
				{
					$prev_page = $limit_start - $limit;
					$next_page = $limit_start + $limit;
					if($prev_page < 0) $prev_page = 0;
					
					$output .= "\n<tfoot><tr class=\"cache_record_".intval($r & 1)."\"><td colspan=\"10\">\n";
					$output .= '	<table style="width: 100%;">';
					$output .= '		<tr>';
					$output .= '			<td style="width: 100px;">';
					if($limit_start > 0)
						$output .= '			<a href="javascript: void(0);" onclick="sl_url_cache.invalidateRecords('.$prev_page.');">&lt; Previous Page</a>';
					$output .= '			</td>';
					$output .= '			<td style="text-align: center;">';
					$output .= '				Records Per Page: ';
					$output .= '				<select onchange="sl_url_cache.updateLimit(this); sl_url_cache.invalidateRecords(0);">';
					$output .= '					<option '.(($limit == 5)    ? 'SELECTED' : '').' value="5">5</option>';
					$output .= '					<option '.(($limit == 10)   ? 'SELECTED' : '').' value="10">10</option>';
					$output .= '					<option '.(($limit == 15)   ? 'SELECTED' : '').' value="15">15</option>';
					$output .= '					<option '.(($limit == 20)   ? 'SELECTED' : '').' value="20">20</option>';
					$output .= '					<option '.(($limit == 25)   ? 'SELECTED' : '').' value="25">25</option>';
					$output .= '					<option '.(($limit == 50)   ? 'SELECTED' : '').' value="50">50</option>';
					$output .= '					<option '.(($limit == 100)  ? 'SELECTED' : '').' value="100">100</option>';
					$output .= '					<option '.(($limit == 200)  ? 'SELECTED' : '').' value="200">200</option>';
					$output .= '					<option '.(($limit == 500)  ? 'SELECTED' : '').' value="500">500</option>';
					$output .= '					<option '.(($limit == 1000) ? 'SELECTED' : '').' value="1000">1000</option>';
					$output .= '				</select>';
					$output .= '			</td>';
					$output .= '			<td>';
					$output .= '				'.$rs_full_count.' cached items';
					$output .= '			</td>';
					$output .= '			<td>';
					$output .= '				Show Page: <input style="width: 32px;" id="show_page" type="text" value="'.((floor($limit_start / $limit))+1).'" /> of '.ceil($rs_full_count / $limit);
					$output .= '				<input type="submit" value="Go" onclick="sl_url_cache.invalidateRecords('.$limit.' * (document.getElementById(\'show_page\').value - 1));" />';
					$output .= '			</td>';
					$output .= '			<td style="text-align: right; width: 100px;">';
					if($rs_full_count > $next_page)
						$output .= '			<a href="javascript: void(0);" onclick="sl_url_cache.invalidateRecords('.$next_page.');">Next Page &gt;</a>';
					$output .= '			</td>';
					$output .= '		</tr>';
					$output .= '	</table>';
					$output .= "\n</td></tr></tfoot>\n";
				...
				echo "<cache_records>$output</cache_records>";
				$this->db2->stop();
			}
			elseif($what == 'url_cache_remove_selected'){
				$this->load_db();
				
				$ids = preg_replace('/[^0-9,]/','',trim($_POST['ids'], ','));
				$q  = "DELETE FROM `sl_url_cache` WHERE `id` IN ($ids)";
				$q2 = "OPTIMIZE TABLE `sl_url_cache`";
				...
				echo "Selected Removed From Cache";
			}
		    //for tickets
			elseif($what=='inbox'){
		...
		public function db_enabled(){
			return (bool)($this->db2 != null && $this->db2->cid != null);
		}
		public function load_db(){
        	global $wpdb;
	        if(class_exists('JFactory')){
	            $db =& JFactory::GetDBO();
	            $this->db2 = new Guardian($db->_resource);
			} else if(!empty($wpdb) && isset($wpdb->dbh)){
				$this->db2 = new Guardian($wpdb->dbh);
			}
			else{
				$this->load_db_settings();
		        if(!empty($this->db_database) && !empty($this->db_host) && !empty($this->db_password) && !empty($this->db_username)){
	            	$this->db2 = new Guardian($this->db_host, $this->db_username, $this->db_password, $this->db_database);	
				}
			}
		}
		...
		public function SetDBSetting($key, $value){
			if($this->db_enabled()){
				$key   = $this->db2->clean($key);
				$value = $this->db2->clean($value);
				
				//Create table if it doesn't exist
				$this->db2->query("CREATE TABLE IF NOT EXISTS `sl_settings` (
							  `key` varchar(32) NOT NULL DEFAULT '',
							  `value` tinytext NOT NULL,
							  PRIMARY KEY (`key`)
		...
		public function GetDBSetting($key, $default = null){
				if($this->db_enabled()){
					$key = $this->db2->clean($key);
					$rs = $this->db2->query("SELECT `value` FROM `sl_settings` WHERE `key` = '$key'");
					if(count($rs) > 0)
						return $rs[0][0];
					else
						return $default;
		...
		public function QueryDB_Clean($string){
			global $wpdb;
			if($wpdb && isset($wpdb->dbh)){//IS WORDPRESS
				if(gettype($wpdb->dbh) == "object"){//MySqlI
					$wpdb->dbh->real_escape_string($string);
				}
				else{//MySql
					return mysql_real_escape_string($string, $wpdb->dbh);
				}
			}
			else if($this->db && isset($this->db->_resource)){//IS JOOMLA
				if(gettype($this->db->_resource) == "object"){//MySqlI
					return $this->db->_resource->real_escape_string($string);
				}
				else{//MySql
					return mysql_real_escape_string($string, $this->db->_resource);
				}
			}
			
			return mysql_escape_string($string);
		}
		public function QueryDB_Init(){
			if($this->is_WordPress()){
				...
				if(!$wpdb){
					...
					if(!defined('WP_Table_Prefix')) define('WP_Table_Prefix', $table_prefix); 
					
					$file_name = dirname(__FILE__).'/../../../wp-includes/wp-db.php';
		...
		private function acct_form_module(){
						...
						//Spam Filter Settings
						{
							//Add items
							{
								...
								if(isset($_POST['spamEMail_add']) && $_POST['spamEMail_add'] != ""){
									...
									if(count($this->QueryDB("SELECT id FROM !spamfilter WHERE type=1 AND term='".$this->QueryDB_Clean($_POST['spamEMail_add'])."'")) == 0)
										$this->QueryDB("INSERT INTO !spamfilter VALUES(null, ".time().", 1, '"  .$this->QueryDB_Clean($_POST['spamEMail_add'])."')", false);
								}
								//spamUsername_add
								if(isset($_POST['spamUsername_add']) && $_POST['spamUsername_add'] != ""){
									//is it part of the DB?
									if(count($this->QueryDB("SELECT id FROM !spamfilter WHERE type=2 AND term='".$this->QueryDB_Clean($_POST['spamUsername_add'])."'")) == 0)
										$this->QueryDB("INSERT INTO !spamfilter VALUES(null, ".time().", 2, '"  .$this->QueryDB_Clean($_POST['spamUsername_add'])."')", false);
								}
								//spamIP_add
								if(isset($_POST['spamIP_add']) && $_POST['spamIP_add'] != ""){
									//is it part of the DB?
									if(count($this->QueryDB("SELECT id FROM !spamfilter WHERE type=3 AND term='".$this->QueryDB_Clean($_POST['spamIP_add'])."'")) == 0)
										$this->QueryDB("INSERT INTO !spamfilter VALUES(null, ".time().", 3, '"  .$this->QueryDB_Clean($_POST['spamIP_add'])."')", false);
								}
								//spamTerm_add
								if(isset($_POST['spamTerm_add']) && $_POST['spamTerm_add'] != ""){
									//is it part of the DB?
									if(count($this->QueryDB("SELECT id FROM !spamfilter WHERE type=4 AND term='".$this->QueryDB_Clean($_POST['spamTerm_add'])."'")) == 0)
										$this->QueryDB("INSERT INTO !spamfilter VALUES(null, ".time().", 4, '"  .$this->QueryDB_Clean($_POST['spamTerm_add'])."')", false);
								}
							}
							...
							//Remove items
							{
								...
								if(isset($_POST['spamEMail_rem_check']) && isset($_POST['spamFilterEmail_rem']) && count($_POST['spamFilterEmail_rem']) > 0){
									foreach($_POST['spamFilterEmail_rem'] as $id){
										if($remIdList == "")
											$remIdList .= "id=".$this->QueryDB_Clean($id);
										else
											$remIdList .= " OR id=".$this->QueryDB_Clean($id);
									}
								}
								//Username
			...
			//Javascripts
			{
	            ...
	            $output .= '        <script type="text/javascript">
		                                ...
		                                function sl_init(){
		                                    ...
		                                    server_timeout = new sl_slider("server_timeout", 1, 16, '.$this->timeout.');
		                                    cache_exp      = new sl_slider("cache_exp", 1, 365, '.$this->url_cache_expires.');
            ...
            //URL CACHEING
            {
            	#Start module container
            	{
		            ...
		            $output .= '            <div class="s5_w_modwrap">';
		            $output .= '                <div class="s5_backmiddlemiddle2">';
		            $output .= $this->ModuleStart("1", "100", "", "URL Request Cache <span style=\"color: red; font-weight: bold;\">NEW FEATURE</span>", "advert", TRUE);
				}
	            
	            $fail_reason = '';
	            $div_db_info_style = '';
	            $div_url_cache_style = '';
	            $div_url_chache_container_style = '';
	            $url_cache_toggle_text = '';
	            #Find out what page to display
	            {
	            	$is_wp = $this->is_WordPress();
	            	$is_jm = $this->is_joomla();
	            	$this->load_db();
	            	if($this->db_enabled() || $is_wp || $is_jm){
	            		$url_cache_toggle_text = "Disable Caching";	
	            		if($is_wp){
	            			$fail_reason = "This information is not needed for wordpress installations.";
	            			
	            			$div_db_info_style = 'display: none;';
	            			$url_cache_toggle_text = '';
						}
						else if($is_jm){
							$fail_reason = "This information is not needed for joomla installations.";
	            			...
	            			if($conn === false){
	            				$fail_reason = 'This information does not appear correct and will not allow for a connection.';
							}
							else {
	            				$db_select = @mysql_select_db($this->db_database, $conn);
	            				if($db_select === false){
	            					$fail_reason = 'The given database name does not exist.';
								}
								else{
									$fail_reason = false;
								}
							...
							if($fail_reason === false){
								$div_db_info_style = 'display: none;';
							}
							else {
								$div_url_cache_style = 'display: none;';
							}
						}
					}
					else{
						if(isset($this->db_password) && $this->db_password != ''){
							$fail_reason = 'This information does not appear correct and will not allow for a connection.';
							$div_url_cache_style = 'display: none;';
						}
						else{
							$div_url_chache_container_style = 'display: none;';
							$url_cache_toggle_text = "Enable Caching";
							$div_url_cache_style = 'display: none;';
						}
					}
				}
	            ...
	            $radio_mode_0 = ($this->url_cache_mode == 0) ? "CHECKED" : "";
	            $radio_mode_1 = ($this->url_cache_mode == 1) ? "CHECKED" : "";
	            $radio_mode_2 = ($this->url_cache_mode == 2) ? "CHECKED" : "";
	            $radio_mode_3 = ($this->url_cache_mode == 3) ? "CHECKED" : "";
	            $output .= "
	            <div id=\"div_enable_toggle\">
	            	<h3><span class=\"span_link\" style=\"color: blue;\" id=\"url_cache_toggle_link\" onclick=\"sl_url_cache.toggleUrlCacheEnabled();\">$url_cache_toggle_text</span></h3>
	            </div>
	            <div id=\"div_url_chache_container\" style=\"$div_url_chache_container_style\">
		            <div id=\"div_db_info\" style=\"$div_db_info_style\">
	            		<input type=\"hidden\" value=\"\" id=\"url_cache_enabled\" />
	            		<p>Url request caching will allow you to protect your site as fast as possible.  With this feature enabled request made to your site will be first scanned by us and then the result is saved as safe or malicious.  By doing this you will not need to wait for securelive's scanning server to approve the request if it has been made already. The cache will be saved in your database.</p>
	            		<br><br>
	            		<span style=\"color: red; font-weight: bold; padding-bottom: 10px;\">$fail_reason</span>
	            		<br><br>
	            		<table id=\"tbl_db_info\" style=\"width: 100%;\">
	            			<tr>
	            				<td>Host</td>
	            				<td><input value=\"{$this->db_host}\" type=\"text\" id=\"txbxHost\" onchange=\"return sl_url_cache.onDataBaseInfoChanged(this);\" /></td>
	            				
	            				<td>Database</td>
	            				<td><input value=\"{$this->db_database}\" type=\"text\" id=\"txbxDatabase\" onchange=\"return sl_url_cache.onDataBaseInfoChanged(this);\" /></td>
	            				
	            				<td>Username</td>
	            				<td><input value=\"{$this->db_username}\" type=\"text\" id=\"txbxDatabaseUser\" onchange=\"return sl_url_cache.onDataBaseInfoChanged(this);\" /></td>

	            				<td>Password</td>
	            				<td><input value=\"{$this->db_password}\" type=\"password\" id=\"txbxDatabasePassword\" onchange=\"return sl_url_cache.onDataBaseInfoChanged(this);\" /></td>

	            				<td style=\"text-align: right;\"><input type=\"button\" onclick=\"sl_url_cache.onDataBaseInfoSave(this);\" value=\"Save\" /></td>
	            			</tr>
	            		</table>
	            		<div style=\"text-align: right;\" id=\"show_cache_settings\">
	            			<span class=\"span_link\" onclick=\"sl_url_cache.showPage('div_url_cache');\">Show Cache Settings &gt;</span>
	            		</div>
		            </div>
		            <div id=\"div_url_cache\" style=\"$div_url_cache_style\">
		            	<table id=\"tbl_use_cache_when\" style=\"width: 100%;\">
		            		<tr>
		            			<td>
		            				<table>
		            					<tr>
		            						<td style=\"vertical-align: top; font-weight: bold; padding-top: 6px;\">Use cache when: </td>
		            						<td>
		            							<table>
		            								<tr>
		            									<td><input type=\"radio\" name=\"radio_use_cache\" $radio_mode_3 value=\"3\" /> Use cache before trying SecureLive's global scanning server <em>(fastest).</em><br><span style=\"padding-left: 24px;\">Use lockdown mode if global scanning server is unavailable.</span>
		            								</tr>
		            								<tr>
		            									<td><input type=\"radio\" name=\"radio_use_cache\" $radio_mode_2 value=\"2\" /> Use cache before trying SecureLive's global scanning server <em>(fastest).</em><br><span style=\"padding-left: 24px;\"><b>Do not</b> use lockdown mode if global scanning server is unavailable.</span>
		            								</tr>
		            								<tr>
		            									<td><input type=\"radio\" name=\"radio_use_cache\" $radio_mode_1 value=\"1\" /> Use cache only if global scanning server is not available.<br><span style=\"padding-left: 24px;\">Use lockdown mode if global scanning server is unavailable.</span>
		            								</tr>
		            								<tr>
		            									<td><input type=\"radio\" name=\"radio_use_cache\" $radio_mode_0 value=\"0\" /> Use cache only if global scanning server is not available.<br><span style=\"padding-left: 24px;\"><b>Do not</b> use lockdown mode if global scanning server is unavailable.</span>
		            								</tr>
		            							</table>
		            						</td>
		            					</tr>
		            			...
		            			<td>
		            				<table id=\"tbl_url_cache_sliders\" style=\"padding-left: 16px; width: 100%;\">
		            					<tr>
		            						<td style=\"padding-bottom: 10px;\">
		            							<span style=\"float:left;\">Cache Controls</span><br/>
		            							<input style=\"width: 130px;\" id=\"btnClearExp\" type=\"button\" onclick=\"sl_url_cache.onClearExpired();\" value=\"Clear Expired Cache\" />
		            							<input style=\"width: 100px;\" id=\"btnClearAll\" type=\"button\" onclick=\"sl_url_cache.onClearAll();\"     value=\"Clear All Cache\" />
		            						</td>
		            					</tr>
		            					<tr>
		            						<td style=\"padding-bottom: 26px;\">
		            							<span style=\"float:left;\">Global Server Timeout: <em>(seconds)</em></span><br/>
							                    <div style=\"position: relative;float:left;width:auto;\">
							                        <input name=\"server_timeout_int\" id=\"server_timeout_slider_val\" type=\"text\" value=\"{$this->timeout}\" style=\"width:30px;font-size:13px;position:absolute;top:3px;\" />
							                        <div id=\"server_timeout_slider_holder\" class=\"sl_slider_holder\">
							                            <div id=\"server_timeout_slider\" class=\"sl_slider\" style=\"left:-5px;\"></div>
							                        </div>
							                    </div>
		            						</td>
		            					</tr>
		            					<tr>
		            						<td>
		            							<span style=\"float:left;\">Cache Expires: <em>(days)</em></span><br/>
							                    <div style=\"position: relative;float:left;width:auto;\">
							                        <input name=\"cache_exp_int\" id=\"cache_exp_slider_val\" type=\"text\" value=\"{$this->url_cache_expires}\" style=\"width:30px;font-size:13px;position:absolute;top:3px;\" />
							                        <div id=\"cache_exp_slider_holder\" class=\"sl_slider_holder\">
							                            <div id=\"cache_exp_slider\" class=\"sl_slider\" style=\"left:-5px;\"></div>
							                        </div>
							                    </div>
		            						</td>
		            					</tr>
	            		...
	            		<div style=\"text-align: left;\" id=\"show_db_settings\">
	            			<span class=\"span_link\" onclick=\"sl_url_cache.showPage('div_db_info');\">&lt;Show Database Settings</span>
	            			<span class=\"span_link\" onclick=\"sl_url_cache.invalidateRecords(); sl_url_cache.showPage('div_url_cache_records');\" style=\"float: right;\">Show Cache Records &gt;</span>
	            		</div>
	            	</div>
	            	<div id=\"div_url_cache_records\" style=\"display: none;\">
		            	<table id=\"tbl_cache_records\" style=\"width: 100%;\">
		            		<tr>
		            			<thead>
									<tr class=\"cache_record_header\">
										<th style=\"width: 20px;\"></th>
										<th style=\"width: 16px;\">ID</th>
										<th style=\"width: 30px;\">Safe</th>
										<th style=\"width: 30px;\">Hits</th>
										<th style=\"width: 370px;\">URI</th>
										<th style=\"width: 80px;\">Threat Type</th>
										<th style=\"width: 78px;\">Threat Level</th>
										<th style=\"width: 91px;\">Recorded IP</th>
										<th style=\"width: 70px;\">Created</th>
										<th style=\"width: 70px;\">Expires</th>
									</tr>
		            			</thead>
		            			<tbody>
		            				<tr>
		            					<td colspan=\"10\">
		            						<div id=\"div_cache_scroll\" style=\"word-wrap:break-word;\">
		            							<table>
		            								<tbody id=\"tbody_cache_records\">
		            									<td colspan=\"9\">
		            										...
		            									</td>
		            								</tbody>
		            			...
		            			<tbody>
									<tr>
										<td colspan=\"9\" style=\"text-align: right;\"><input style=\"width: 125px;\" type=\"button\" value=\"Remove Selected\" id=\"btnRemoveSelected\" onclick=\"sl_url_cache.onRemoveSelected();\" /></td>
									</tr>
		            			</tbody>
		            		</tr>
		            	</table>
