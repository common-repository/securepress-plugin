...
function clear_append_elm(id, tag, contents){
...
function sl_calendar(container){
    ...
    this.get_attacks = function(){
        ...
        url = sl_url+'/sl_admin.php'+sl_query;
...
function sl_bypass_editor(){
	...
	this.save = function(button){
		...
		*/
        ...
        this.ajax.open("POST", sl_url+'/sl_admin.php'+sl_query, true);
        this.ajax.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        this.ajax.setRequestHeader("Content-length", params.length);
        this.ajax.setRequestHeader("Connection", "close");
        this.ajax.send(params);
...
Validator = {
	...
	validate:function(){
		if(empty(this.text())){
	        ...
	        this.ajax.open("POST", sl_url+'/inc/helper.php'+sl_query, true);
	        this.ajax.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	        this.ajax.setRequestHeader("Content-length", params.length);
	        this.ajax.setRequestHeader("Connection", "close");
	        this.ajax.send(params);
...
SL_Test = {
	...
	test:function(elm){
	    ...
	    this.ajax.open("GET", sl_url+'/sl_admin.php'+sl_query+'&xsecurelivex', true);
	    this.ajax.send(null);
	},
	testResp:function(){
...
SL_Scanner = {
	...
	reset:function(img,where){
	    ...
	    this.ajax.open("POST", sl_url3+'sl_scanner.php'+sl_query, true);
	    this.ajax.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	    this.ajax.setRequestHeader("Content-length", params.length);
	    this.ajax.setRequestHeader("Connection", "close");
	    this.ajax.send(params);
	...
	updateFileCount:function(){
	    ...
	    this.fileCountAjax.open("POST", sl_url3+'sl_scanner.php'+sl_query, true);
	...
	openFile:function(file,line){
	    ...
	    this.fileAjax.open("POST", sl_url3+'sl_scanner.php'+sl_query, true);
	...
	startFileScan:function(){
	    ...
	    this.ajax.open("POST", sl_url3+'sl_scanner.php?'+sl_query+'&what=timestamp'+extra, true);
	    this.ajax.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	    this.ajax.setRequestHeader("Content-length", params.length);
	    this.ajax.setRequestHeader("Connection", "close");
	    this.ajax.send(params);
	...
	getBase64:function(button,which){
	    ...
	    this.base64Ajax.open("POST", sl_url3+'sl_scanner.php'+sl_query, true);
...
sl_url_cache = {
	...
	onClearExpired: function(){
		if(sl_url_cache.clear_busy == true) { alert("Cache is busy with another request."); return; }

		var sender = document.getElementById('btnClearExp');
		onClearExp_OrgText = sender.value;
		sender.value = "Clearing...";
		sender.onclick = function(){return;};
		
		var q = 'sl_page=ajax&what=url_cache_clear_exp';
		var xmlhttp;
		...
		xmlhttp.onreadystatechange=function(){
			if (xmlhttp.readyState == 4 && xmlhttp.status == 200){
				sl_url_cache.clear_busy = false;
				var sender = document.getElementById('btnClearExp');
				sender.value = onClearExp_OrgText;
				sender.onclick = function(){ sl_url_cache.onClearExpired(this); };
				return;
			}
		...
		xmlhttp.open("POST", sl_url+'/sl_admin.php'+sl_query, true);
		xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
		xmlhttp.setRequestHeader("Content-length", q.length);
		xmlhttp.setRequestHeader("Connection", "close");
		sl_url_cache.clear_busy = true;
		xmlhttp.send(q);
	},
	onClearAll: function(sender){
		if(sl_url_cache.clear_busy == true) { alert("Cache is busy with another request."); return; }
		
		var sender = document.getElementById('btnClearAll');
		onClearAll_OrgText = sender.value;
		sender.value = "Clearing...";
		sender.onclick = function(){return;};
		
		
		var q = 'sl_page=ajax&what=url_cache_clear_all';
		var xmlhttp;
		if (window.XMLHttpRequest)
			xmlhttp=new XMLHttpRequest();
		else
		...
		xmlhttp.onreadystatechange=function(){
			if (xmlhttp.readyState == 4 && xmlhttp.status == 200){
				sl_url_cache.clear_busy = false;
				var sender = document.getElementById('btnClearAll');
				sender.value = onClearAll_OrgText;
				sender.onclick = function(){ sl_url_cache.onClearAll(this); };
				return;
			}
			else if(xmlhttp.readyState == 4){
				sl_url_cache.clear_busy = false;
	...
	onRemoveSelected: function(){
		...
		var sender = document.getElementById('btnRemoveSelected');
		onClearExp_OrgText = sender.value;
		sender.value = "Removing...";
		sender.onclick = function(){return;};
		
		
		var q = 'sl_page=ajax&what=url_cache_remove_selected&ids=';
		
		var cbs = document.getElementsByName("cache_ids");
		for(var i = 0; i < cbs.length; i++){
			if(cbs[i].checked){
				q += cbs[i].value.toString()+",";
			}
		}
		...
		xmlhttp.onreadystatechange=function(){
			if (xmlhttp.readyState == 4 && xmlhttp.status == 200){
				...
				var sender = document.getElementById('btnRemoveSelected');
				sender.value = onClearExp_OrgText;
				sender.onclick = function(){ sl_url_cache.onRemoveSelected(); };
				sl_url_cache.invalidateRecords();
				return;
			}
			else if(xmlhttp.readyState == 4){
				sl_url_cache.clear_busy = false;
				...
				sender.value = onClearExp_OrgText;
				sender.onclick = function(){ sl_url_cache.onRemoveSelected(); };
				alert('Failed to remove from cache, please try again.');
				return;
			}
		}
	...
	onDataBaseInfoChanged: function(sender){
	
	},
	onDataBaseInfoSave: function(sender){
		if(sender) sender.value = 'Saving...';
		
		if(sl_url_cache.save_busy == true){
			alert('Save in progress please wait.');
			return;	
		}
		
		//Build Query
		var q = 'sl_page=ajax&what=url_cache_save';
		q += '&host='     + escape(document.getElementById('txbxHost').value);
		q += '&database=' + escape(document.getElementById('txbxDatabase').value);
		q += '&username=' + escape(document.getElementById('txbxDatabaseUser').value);
		q += '&password=' + escape(document.getElementById('txbxDatabasePassword').value);
		q += '&timeout='  + escape(document.getElementById('server_timeout_slider_val').value);
		q += '&expires='  + escape(document.getElementById('cache_exp_slider_val').value);
		
		var elms = document.getElementsByName('radio_use_cache');
		for( i = 0; i < elms.length; i++ ){
			if(elms[i].checked == true){
				q += '&when=' + escape(elms[i].value);
				break;
			}
		}
		...
		xmlhttp.onreadystatechange=function(){
			if (xmlhttp.readyState == 4 && xmlhttp.status == 200){
				sl_url_cache.save_busy = false;
				sl_gateway.refresh('acct_form');
				return;
			}
			else if(xmlhttp.readyState == 4){
				sl_url_cache.save_busy = false;
				alert('Failed to save url cache settings, please try again.');
				return;
			}
		}
		...
		xmlhttp.setRequestHeader("Content-length", q.length);
		xmlhttp.setRequestHeader("Connection", "close");
		sl_url_cache.save_busy = true;
		xmlhttp.send(q);

	},
	showPage: function(page_id){
		var elm_a = document.getElementById('div_db_info');
		var elm_b = document.getElementById('div_url_cache');
		var elm_c = document.getElementById('div_url_cache_records');
		var elm_z = document.getElementById(page_id);
		
		elm_a.style.display = 'none';
		elm_b.style.display = 'none';
		elm_c.style.display = 'none';
		elm_z.style.display = 'block';
	},
	toggleUrlCacheEnabled: function(){
		var tLink = document.getElementById('url_cache_toggle_link');
		var elm   = document.getElementById('div_url_chache_container');
		
		if(elm.style.display == 'none'){//Enable
			tLink.innerHTML = 'Disable Caching';
			elm.style.display = 'block';
		}
		else{//Disable
			if(sl_url_cache.save_busy){
				alert('Save in progress please wait.');
				return;	
			}
			
			if(!confirm("Are you sure that you want to disable caching?  This could result in your site being slowed down.")){
				return;
			}
			
			tLink.innerHTML = 'Enable Caching';
			elm.style.display = 'none';
			
			document.getElementById('txbxHost').value = '';
			document.getElementById('txbxDatabase').value = '';
			document.getElementById('txbxDatabaseUser').value = '';
			document.getElementById('txbxDatabasePassword').value = '';
			
			sl_url_cache.onDataBaseInfoSave(null);
		}
		
	},
	invalidateRecords: function(start){
		if(!start) start = 0;
		
		clear_append_elm("div_cache_scroll", "table", "<tr><td>Reading Cache Data...</td></tr>");
		
		var q = 'sl_page=ajax&what=url_cache_records&Page='+escape(start);
		var xmlhttp;
		if (window.XMLHttpRequest)
			xmlhttp=new XMLHttpRequest();
		else
		...
		xmlhttp.onreadystatechange=function(){
			if (xmlhttp.readyState == 4 && xmlhttp.status == 200){
				var start = xmlhttp.responseText.indexOf("<cache_records>");
				if(start == -1){
					clear_append_elm("div_cache_scroll", "table", "<tr><td colspan=\"9\">Connection Failed.<span class=\"span_link\" onclick=\"sl_url_cache.invalidateRecords();\"> Try Again?</span></td></tr>");
					return;
				}
				var end = xmlhttp.responseText.indexOf("</cache_records>", start);
				if(end == -1) end = xmlhttp.responseText.length;

				clear_append_elm("div_cache_scroll", "table", xmlhttp.responseText.substring(start + 15, end));
				return;
			}
			else if(xmlhttp.readyState == 4){
				clear_append_elm("div_cache_scroll", "table", "<tr><td colspan=\"9\">Connection Failed.<span class=\"span_link\" onclick=\"sl_url_cache.invalidateRecords();\"> Try Again?</span></td></tr>");
	...
	updateLimit: function(sender){
		var limit = sender.options[sender.selectedIndex].value;
		var now = new Date();
		now.setDate(now.getDate() + 30);
		document.cookie= "limit=" + escape(limit.toString()) + ";expires="+now.toUTCString();
	}
}